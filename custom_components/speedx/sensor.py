"""Sensor platform for the SpeedX parcel tracker integration."""
...
from .coordinator import SpeedXCoordinator
...
async def async_setup_entry(
    hass: HomeAssistant,
    entry: SpeedXConfigEntry,
    async_add_entities: AddEntitiesCallback,
) -> None:
    """Set up SpeedX sensor entities from a config entry."""
    ...
    entities: list[SensorEntity] = [
        SpeedXIncomingParcelsSensor(
            coordinator, entry, async_add_entities, current_barcodes
        ),
    ]
    for parcel in coordinator.data or []:
        entities.append(
            SpeedXParcelSensor(coordinator, entry, parcel.get("barcode", ""))
        )
    entities.append(SpeedXNextDeliverySensor(coordinator, entry))
    entities.append(SpeedXDeliveredParcelsSensor(coordinator, entry))
    entities.append(SpeedXLastUpdateSensor(coordinator, entry))

    async_add_entities(entities)


class SpeedXIncomingParcelsSensor(
    CoordinatorEntity[SpeedXCoordinator], SensorEntity
):
    ...
    def _handle_coordinator_update(self) -> None:
        ...
        if new_barcodes:
            self._async_add_entities(
                SpeedXParcelSensor(self.coordinator, self._entry, barcode)
...
class SpeedXParcelSensor(CoordinatorEntity[SpeedXCoordinator], SensorEntity):
    """Per-parcel sensor reporting the status of one tracked SpeedX parcel."""
    ...
    def __init__(
        self, coordinator: SpeedXCoordinator, entry: ConfigEntry, barcode: str
    ) -> None:
        """Initialize the sensor."""
        super().__init__(coordinator)
        self._entry = entry
...
class SpeedXNextDeliverySensor(
    CoordinatorEntity[SpeedXCoordinator], SensorEntity
):
    ...
    def __init__(
        self, coordinator: SpeedXCoordinator, entry: ConfigEntry
    ) -> None:
        """Initialize the sensor."""
        super().__init__(coordinator)
...
class SpeedXDeliveredParcelsSensor(
    CoordinatorEntity[SpeedXCoordinator], SensorEntity
):
    """Recently delivered tracked SpeedX parcels."""
...
class SpeedXLastUpdateSensor(
    CoordinatorEntity[SpeedXCoordinator], SensorEntity
):
    """Diagnostic sensor reporting when SpeedX was last polled successfully."""
