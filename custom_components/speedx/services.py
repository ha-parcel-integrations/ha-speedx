"""Services for the SpeedX parcel tracker integration.

`speedx.track_parcel` / `speedx.untrack_parcel` let you add or remove a
...
def _resolve_entry(hass: HomeAssistant):
    """Return the single SpeedX hub, or raise when it is not set up."""
    entries = hass.config_entries.async_entries(DOMAIN)
    if not entries:
        raise ServiceValidationError("SpeedX is not set up")
    return entries[0]


def async_setup_services(hass: HomeAssistant) -> None:
    """Register the SpeedX services (idempotent)."""
    ...
    async def _track(call: ServiceCall) -> None:
        ...
        if not valid_tracking_code(tracking_code):
            raise ServiceValidationError(
                f"'{tracking_code}' is not a valid SpeedX tracking code"
            )
...
def async_unload_services(hass: HomeAssistant) -> None:
    """Remove the SpeedX services (single-entry integration, so on unload)."""
