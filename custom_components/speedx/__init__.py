"""SpeedX parcel tracker custom component for Home Assistant."""
...
from .api import SpeedXApiClient
...
@dataclass
class SpeedXData:
    """Runtime data attached to the SpeedX config entry."""

    client: SpeedXApiClient
    coordinator: SpeedXCoordinator


type SpeedXConfigEntry = ConfigEntry[SpeedXData]


async def async_setup_entry(hass: HomeAssistant, entry: SpeedXConfigEntry) -> bool:
    """Set up SpeedX from a config entry."""
    # No auth: SpeedX tracking is public, so the HA-managed session is fine.
    client = SpeedXApiClient(async_get_clientsession(hass))
    coordinator = SpeedXCoordinator(hass, client, entry)
    ...
    entry.runtime_data = SpeedXData(client=client, coordinator=coordinator)
...
async def async_unload_entry(hass: HomeAssistant, entry: SpeedXConfigEntry) -> bool:
    """Unload the SpeedX config entry."""
