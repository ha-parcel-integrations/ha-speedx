"""Calendar platform for the SpeedX parcel tracker integration."""
...
async def async_setup_entry(
    ...
    """Set up the SpeedX deliveries calendar from a config entry."""
    async_add_entities([SpeedXDeliveriesCalendar(entry.runtime_data.coordinator, entry)])


class SpeedXDeliveriesCalendar(CoordinatorEntity[SpeedXCoordinator], CalendarEntity):
    """A read-only calendar of expected SpeedX deliveries.
    ...
    def __init__(self, coordinator: SpeedXCoordinator, entry: ConfigEntry) -> None:
    ...
    def _events(self) -> list[CalendarEvent]:
        ...
        for parcel in self.coordinator.data or []:
            ...
            summary = sender or (f"Parcel {barcode}" if barcode else "SpeedX parcel")
            description_parts = [
