"""Tests for SpeedX sensor property logic."""
from datetime import datetime, timezone
from unittest.mock import MagicMock

from custom_components.speedx.const import ParcelStatus
from custom_components.speedx.sensor import (
    SpeedXDeliveredParcelsSensor,
    SpeedXIncomingParcelsSensor,
    SpeedXLastUpdateSensor,
    SpeedXNextDeliverySensor,
    SpeedXParcelSensor,
)
...
def test_incoming_counts_and_lists():
    ...
    sensor = SpeedXIncomingParcelsSensor(coordinator, _entry(), lambda _: None, set())
...
def test_parcel_sensor_status_and_attributes():
    ...
    sensor = SpeedXParcelSensor(_coordinator([parcel]), _entry(), "A")
...
def test_parcel_sensor_missing_barcode():
    sensor = SpeedXParcelSensor(_coordinator([_parcel("A")]), _entry(), "OTHER")
...
def test_next_delivery_picks_earliest():
    ...
    sensor = SpeedXNextDeliverySensor(coordinator, _entry())
...
def test_next_delivery_none_without_moments():
    sensor = SpeedXNextDeliverySensor(_coordinator([_parcel("A")]), _entry())
    assert sensor.native_value is None
    assert sensor.extra_state_attributes == {}
...
def test_delivered_sensor():
    ...
    sensor = SpeedXDeliveredParcelsSensor(coordinator, _entry())
...
def test_last_update_sensor():
    ...
    sensor = SpeedXLastUpdateSensor(coordinator, _entry())
