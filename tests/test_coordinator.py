"""Tests for the SpeedX coordinator: fetching, caching and events.
...
from custom_components.speedx.api import SpeedXApiError
from custom_components.speedx.const import (
...
from custom_components.speedx.coordinator import SpeedXCoordinator
...
def _in_transit(code: str = ACTIVE_CODE) -> dict:
    ...
    sample["events"][0]["eventDescription"] = "Moving through network"
    return sample
...
async def test_update_keeps_cached_payload_on_error(hass):
    ...
    client.async_get_parcel.side_effect = SpeedXApiError("HTTP 500")
...
async def test_fires_delivery_time_changed_event(hass):
    ...
    assert events == []  # EDD semantics are deliberately not published yet.
