# Automatic parcel tracking from e-mail (IMAP → SpeedX)

Companion guide for [`track_parcels_from_email.yaml`](track_parcels_from_email.yaml): watch your mailbox(es) for shipping e-mails, extract the SpeedX tracking code, and register it with `speedx.track_parcel` — fully automatic, no extra custom component required.

SpeedX is a **code-based** carrier: it has no account inbox, so every parcel must be registered by its tracking code before the integration can follow it. This recipe automates exactly that step.

**How it works, in one line:** the core [IMAP integration](https://www.home-assistant.io/integrations/imap/) fires an `imap_content` event for every new e-mail (including the body); the automation extracts the tracking code — a cheap regex first, an optional AI fallback for everything else — and calls `speedx.track_parcel`.

```
new e-mail ──imap_content──▶ automation ──▶ regex match? ──▶ speedx.track_parcel
                                     │
...
- This integration, with the `speedx.track_parcel` action available (field `tracking_code`).
...
```
(?<![0-9A-Za-z])(?:EXAMPLE|EXA)[0-9A-Za-z]{6,}
```

Two prefixes are covered: **`EXAMPLE…`** and **`EXA…`** — *TODO(carrier): replace these with the prefixes this carrier's codes actually use*. The integration itself accepts any code matching `^[A-Z0-9]{6,30}$`, but matching *that* broadly against e-mail text would hit every order number in the mail — so the example anchors on known prefixes instead. **If your mails carry a different prefix, add it to the alternation** (`(?:EXAMPLE|EXA|YOURPREFIX)`); the AI fallback will surface unknown formats in the meantime.
...
curl -X POST -H "Authorization: Bearer $HA_TOKEN" -H "Content-Type: application/json" \
  ...
  -d '{"sender":"noreply@speedxshipping.com","subject":"Your parcel is on its way",
       "text":"Tracking code: EXAMPLE123456","initial":true,"folder":"INBOX","username":"test"}'
```

Then `speedx.untrack_parcel` the test code afterwards. For a full end-to-end test, forward a real shipping mail to the watched mailbox — it must arrive **unread**.
